...
class GraphAttentionLayerV2(nn.Module):
    ...
    def __init__(self, in_features, out_features, dropout, alpha, concat=True):
        ...
        self.W = nn.Parameter(torch.zeros(size=(2*in_features, out_features)))
        nn.init.xavier_uniform_(self.W.data, gain=1.414)
    ...
    def forward(self, x, adj):
        ...
        # gatv2
        h1 = torch.matmul(x, self.W[:self.in_features, :])
        h2 = torch.matmul(x, self.W[self.in_features:, :])
        e = self.leakyrelu(torch.matmul(h1,self.a)+torch.matmul(h2,self.a).transpose(2,1))
        ...
        attention = F.softmax(attention, dim=1)  # softmax形状保持不变 [N, N]，得到归一化的注意力权重！
        attention = F.dropout(attention, self.dropout, training=self.training)  # dropout，防止过拟合
        h_prime = torch.matmul(attention, h1+h2)  # [N, N].[N, out_features] => [N, out_features]
        # 得到由周围节点通过注意力权重进行更新的表示
        if self.concat:
            return F.elu(h_prime)
        else:
