...
from lab.src.net import GNNStack, MST_DGAT
...
def main_work(args):
    ...
    model = MST_DGAT(gnn_model_type=args.arch, num_layers=args.num_layers, dropout=args.dropout,
                     ...
                     seq_len=seq_length, N=num_nodes, num_classes=num_classes, batch_size=args.batch_size)
    # print & log
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if (epoch+1)%10 == 0 or epoch==0:
            plt.clf()
            plt.imshow(adj.cpu().detach().numpy()[0, :,:], cmap='Blues')
            plt.colorbar()            
